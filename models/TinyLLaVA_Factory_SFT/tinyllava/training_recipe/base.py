...
class BaseTrainingRecipe:
    ...
    def __call__(self, model):
        ...
        model.config.tune_type_text_vision_tower = self.training_arguments.tune_type_text_vision_tower
        model.config.tune_type_text_vision_connector = self.training_arguments.tune_type_text_vision_connector
        return model
    ...
    def add_args(self, model_args):
        ...
        if self.training_arguments.pretrained_model_path is not None:
            ...
            model_args['text_vision_tower'].update(dict(pretrained_text_vision_tower_path=os.path.join(self.training_arguments.pretrained_model_path, 'text_vision_tower')))
            if 'text_vision_connector' not in model_args or model_args['text_vision_connector'] is None:
                model_args['text_vision_connector'] = {}
            model_args['text_vision_connector'].update(dict(pretrained_text_vision_connector_path=os.path.join(self.training_arguments.pretrained_model_path, 'text_vision_connector')))
        return model_args
    ...
    def tune_type_setting(self, model):
        ...
        model = self._text_vision_tower_tune_type_setting(model)
        model = self._text_vision_connector_tune_type_setting(model)
        return model    
    ...
    def _text_vision_tower_tune_type_setting(self, model):
        tune_type = self.training_arguments.tune_type_text_vision_tower.lower()
        assert tune_type in ('frozen', 'full', 'lora', 'qlora'), f'tune_type {tune_type} not supported in this training recipe!'
        if tune_type == 'full':
            model.text_vision_tower.requires_grad_(True)
        elif tune_type == 'frozen':
            model.text_vision_tower.requires_grad_(False)
        return model
    ...
    def _text_vision_connector_tune_type_setting(self, model):
        tune_type = self.training_arguments.tune_type_text_vision_connector.lower()
        assert tune_type in ('frozen', 'full', 'lora', 'qlora'), f'tune_type {tune_type} not supported in this training recipe!'
        if tune_type == 'full':
            for p in model.text_vision_connector.parameters():
                p.requires_grad = True
        elif tune_type == 'frozen':
            for p in model.text_vision_connector.parameters():
                p.requires_grad = False
    ...
    def save(self, model, trainer):
        ...
        print(f'trainer.args.local_rank: {trainer.args.local_rank}')
        print(f'output path: {self.training_arguments.output_dir}')
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            ...
            if language_model_state_dict:
                torch.save(language_model_state_dict, language_model_output_path)
                print(f"Model saved successfully at {language_model_output_path}")
            else:
                print("Error: language_model_state_dict is empty.")
            if hasattr(model.config.text_config, 'save_pretrained'):
                model.config.text_config.save_pretrained(language_model_output_dir, from_pt=True)
            else:
                print("Error: model.config.text_config does not have save_pretrained method.")
            model.config.text_config.save_pretrained(language_model_output_dir, from_pt=True)
        #save vision tower
        print('saving vision tower\n')
        ...
        #save text vision tower
        print('saving text vision tower\n')
        text_vision_tower_state_dict = get_state_maybe_zero_3(model.text_vision_tower.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            text_vision_tower_output_dir = os.path.join(self.training_arguments.output_dir, 'text_vision_tower')
            os.makedirs(text_vision_tower_output_dir, exist_ok=True)
            text_vision_tower_output_path = os.path.join(self.training_arguments.output_dir, 'text_vision_tower/pytorch_model.bin')
            torch.save(text_vision_tower_state_dict, text_vision_tower_output_path)
            if isinstance(model.vision_tower, PreTrainedModel):
                model.text_vision_tower.config.save_pretrained(text_vision_tower_output_dir, from_pt=True)
        # save text vision connector
        print('saving text vision connector\n')
        text_vision_connector_state_dict = get_state_maybe_zero_3(model.text_vision_connector.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            text_vision_connector_output_dir = os.path.join(self.training_arguments.output_dir, 'text_vision_connector')
            os.makedirs(text_vision_connector_output_dir, exist_ok=True)
            text_vision_connector_output_path = os.path.join(self.training_arguments.output_dir, 'text_vision_connector/pytorch_model.bin')
            torch.save(text_vision_connector_state_dict, text_vision_connector_output_path)
    ...
    def load(self, model, model_args={}):
        if not ('lora' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json'))): # loading model for non-lora/non-qlora pretraining
            ...
            # print(f'FIRST: Loading vision_tower from {model_args["vision_tower"]}')
            model.load_connector(**model_args['connector'])
            # print(f'FIRST: Loading text vision tower from {model_args["text_vision_tower"]}')
