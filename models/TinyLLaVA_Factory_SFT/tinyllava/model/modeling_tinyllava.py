...
class TinyLlavaUniForConditionalGeneration(TinyLlavaForConditionalGeneration):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # Put all images in a list, cause in each training data the number of synthesized images might be different
        image_counts = []
        all_images = []
        if isinstance(images, torch.Tensor):
            if len(images.shape) == 5: # [B, N, 3, 384, 384]
                batch_size = images.size(0)
                for i in range(batch_size):
                    num_images = images[i].size(0)
                    image_counts.append(num_images)
                    all_images.extend([img for img in images[i]])
            else:                       # [B, 3, 1024, 1024] pretrain stage
                image_counts.extend([1] * images.size(0))
                all_images.extend([img for img in images])
        else: # List
            for sample_imgs in images:
                if len(sample_imgs.shape) == 3: # sigal image [3, 1024, 1024]
                    image_counts.append(1)
                    all_images.append(sample_imgs)
                else: # Multipal images [N, 3, 1024, 1024]
                    num_images = sample_imgs.size(0)
                    image_counts.append(num_images)
                    all_images.extend([img for img in sample_imgs])
        ...
        image_features = self.encode_images(stacked_images)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                cur_new_labels.append(cur_labels_noim[i])
                if i < num_images:
                    assert sample_indices[cur_image_idx] == batch_idx, f'Image index mismatch: sample_indices[{cur_image_idx}]={sample_indices[cur_image_idx]} != batch_idx={batch_idx}'
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_labels.append(torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
        ...
        # print(f'cur input length: {new_input_embeds[0].shape}')
        # print(f'tokenizer max length: {tokenizer_model_max_length}')
        # Truncate sequences to max length as image embeddings can make the sequence longer
        tokenizer_model_max_length = getattr(self.config, 'tokenizer_model_max_length', None)
