...
python eval/eval_for_api.py \
    ...
    --answers_path $EVAL_DIR/pope/answers_prompt${PROMPT_ID}/$METHOD_FOLDER/${MODEL_NAME} \
    --temperature 0 \
    --direction $DIRECTION \
...
output_file=$EVAL_DIR/pope/answers_prompt${PROMPT_ID}/$METHOD_FOLDER/${MODEL_NAME}/merge.jsonl
...
for IDX in $(seq 0 $((THREAD_NUM-1))); do
    cat $EVAL_DIR/pope/answers_prompt${PROMPT_ID}/$METHOD_FOLDER/${MODEL_NAME}/thread_${IDX}.jsonl >> "$output_file"
done

python eval/eval_pope.py \
    --annotation-dir $EVAL_DIR/pope/coco \
    --question-file $EVAL_DIR/pope/llava_pope_test.jsonl \
    --result-file $EVAL_DIR/pope/answers_prompt${PROMPT_ID}/$METHOD_FOLDER/${MODEL_NAME}/merge.jsonl \
